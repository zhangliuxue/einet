...
char *cconvert(const char *from_charset, const char *to_charset, const char *input) {
	...
	if ((cd = iconv_open(to_charset, from_charset)) == (iconv_t) -1)
		return NULL;
#ifdef _WIN32
	if (stricmp(from_charset, "wchar_t") == 0 || stricmp(from_charset, "UTF-16LE") == 0) 
#else
	if (strcasecmp(from_charset, "wchar_t") == 0 || strcasecmp(from_charset, "UTF-16LE") == 0) 
#endif
	{
		inleft = wcslen((wchar_t*) input) * sizeof(wchar_t);
	} else {
		inleft = strlen(input);
	}
	inbuf = input;
...
string Unicode_To_UTF8(const wchar_t * s) {
	if (s == NULL)
		return "";

	char *p = cconvert("UTF-16LE", "UTF-8", (char*) s);
	if(p==NULL)
				return "";
	std::string wstr = p;
	free(p);
